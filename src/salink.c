...
static const char *prv_get_label_str_e(uint8_t id, uint8_t label_type)
{
	if (label_type & 1)
...
static uint8_t prv_add_label_e(salink_obj_t *obj, uint16_t size,
			       uint8_t type, uint8_t id)
{
	...
	if (str[0] >= 'A' && str[0] <= 'Z') {
		...
		for (i = obj->label_start; i < label_count; i++) {
			...
			str1 = prv_get_label_str_e(labels[i].id,
						   labels[i].type);
			if (err_type != SPECASM_ERROR_OK)
				return 0;
...
static void prv_add_equ_label_e(specasm_line_t *line, salink_obj_t *obj)
{
	...
	type = op_code[0] == SPECASM_LINE_TYPE_SL ?
		SALINK_LABEL_TYPE_EQU_SHORT :
		SALINK_LABEL_TYPE_EQU_LONG;
	is_global = prv_add_label_e(obj, 0, type, op_code[1]);
	if (err_type != SPECASM_ERROR_OK)
		return;
...
static void prv_parse_obj_e(const char *fname)
{
	...
	for (i = 0; i < state.lines.num_lines; i++) {
		...
		if ((line->type == SPECASM_LINE_TYPE_LL) ||
			...
			(void) prv_add_label_e(obj, size, type,
					       line->data.label);
		} else if (line->type == SPECASM_LINE_TYPE_EQU) {
			prv_add_equ_label_e(line, obj);
				       ...
				       salink_obj_t *obj)
{
	...
	for (i = obj->label_start; i < obj->label_end; i++) {
		...
		if (lng)
			lab_str = specasm_state_get_long_e(label->id);
		else
			lab_str = specasm_state_get_short_e(label->id);
		if (err_type != SPECASM_ERROR_OK)
			return 0;
...
static void prv_resolve_address_e(salink_obj_t *obj,
				  specasm_line_t *line,
...
static int16_t prv_eval_exp_from_id_e(salink_obj_t *obj,
				      unsigned int line_no, uint8_t id,
				      uint8_t lng)
{
	const char *str;
	...
	str = prv_get_label_str_e(id, lng);
	if (err_type != SPECASM_ERROR_OK)
		return 0;

	exp = salink_equ_eval_e(obj, str);

	if ((err_type != SPECASM_ERROR_OK) && (err_type < SPECASM_MAX_ERRORS)) {
		snprintf(error_buf, sizeof(error_buf),
			 "%s:%d %s", obj->fname, line_no,
					   ...
					   uint16_t offset)
{
	...
	uint8_t lng = addr_type == SPECASM_FLAGS_ADDR_LONG ?
		SALINK_LABEL_TYPE_LNG : SALINK_LABEL_TYPE_SHORT;
				    ...
				    unsigned int l, uint8_t id_pos)
{
	...
	id1 = op_code[0];
	id2 = op_code[1];
	lng = op_code[2] == SPECASM_FLAGS_ADDR_LONG ?
		SALINK_LABEL_TYPE_LNG : SALINK_LABEL_TYPE_SHORT;
				...
				unsigned int line_no, uint8_t loc)
{
	...
	exp = prv_eval_equ_label(line, obj, line_no,
				 line->data.op_code[loc]);
	if (err_type != SPECASM_ERROR_OK)
		return;

	if ((exp > 255) || (exp < -128)) {
		snprintf(error_buf, sizeof(error_buf),
			 "%s:%d immediate too big :%d", obj->fname,
			 line_no, exp);
		err_type = SALINK_ERROR_SIZE_TOO_BIG;
		return;
	}
		...
		case SPECASM_LINE_TYPE_LD:
			...
			if ((addr_type == SPECASM_FLAGS_ADDR_SHORT) ||
				...
				lng = addr_type == SPECASM_FLAGS_ADDR_LONG ? 1
									   : 0;
				prv_resolve_address_e(obj, line, i, id,
						      addr, lng);
			}
			break;
