...
static void specasm_dump_line_e(unsigned int l, uint8_t r, uint8_t inv)
{
	...
	if ((line->type == SPECASM_LINE_TYPE_LL) ||
		...
		if (line->data.bad_comment && !inv)
		   ...
		   ((line->type >= SPECASM_LINE_TYPE_EQUB) &&
...
static void prv_selecting_copy(void);
...
static char *prv_complete_filename_e(char *com, uint8_t len)
{
	...
	if ((count == 1) && (com[len] == 'x') && (com[len - 1] == '.'))
		return com;
...
static void prv_goto_line(unsigned int target)
{
	unsigned int starting_line;
